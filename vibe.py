...
def get_vibe_results(pred_cam, pred_verts, pred_pose, pred_betas, pred_joints3d, joints2d_lis, bbox_lis, frame_lis,
                     orig_dim, person_id):
...
def main(args):
    ...
    mot = MPT(device=device, batch_size=TRACKER_BATCH_SIZE, display=False, detector_type=args.detector,
              output_format='dict', yolo_img_size=yolo_img_size, )
    ...
    path_to_input_pipe = "in.pipe1"
    capturing_width = 480
    capturing_height = 320
    color_depth = 3
    print("Running capturer!")
    print(cv2.getBuildInformation())
    print(f"We are about to open [{path_to_input_pipe}] ")
    frame = -1
    while (True):
        # If q is pressed cap.stop will turn True
        # if(cap.stop):
        #	break
        data = file.read(capturing_width * capturing_height * color_depth)
        if len(data) == 0: continue
    a = np.frombuffer(data, dtype=np.uint8)
    images = a.reshape((capturing_height, capturing_width, 3))
    ...
    if len(tracking_results.keys()) != 0:
        ...
        with torch.no_grad():
            ...
            pred_pose.append(
                output['theta'][:, -MIN_NUM_FRAMES:, ][:, :, 3:75].reshape(batch_size * MIN_NUM_FRAMES, -1))
            pred_betas.append(
                output['theta'][:, -MIN_NUM_FRAMES:, ][:, :, 75:].reshape(batch_size * MIN_NUM_FRAMES, -1))
            pred_joints3d.append(output['kp_3d'][:, -MIN_NUM_FRAMES:, ].reshape(batch_size * MIN_NUM_FRAMES, -1, 3))

            del batch

        pred_verts[-MIN_NUM_FRAMES:], pred_cam[-MIN_NUM_FRAMES:], pred_pose[-MIN_NUM_FRAMES:], pred_betas[
                                                                                               -MIN_NUM_FRAMES:], pred_joints3d[
                                                                                                                  -MIN_NUM_FRAMES:], norm_joints2d[
                                                                                                                                     -MIN_NUM_FRAMES:] = temporal_simplify(
            pred_verts[-MIN_NUM_FRAMES:], pred_cam[-MIN_NUM_FRAMES:], pred_pose[-MIN_NUM_FRAMES:],
            pred_betas[-MIN_NUM_FRAMES:], pred_joints3d[-MIN_NUM_FRAMES:], norm_joints2d[-MIN_NUM_FRAMES:], device,
            args)

        get_vibe_results(pred_cam[-MIN_NUM_FRAMES:], pred_verts[-MIN_NUM_FRAMES:], pred_pose[-MIN_NUM_FRAMES:],
                         pred_betas[-MIN_NUM_FRAMES:], pred_joints3d[-MIN_NUM_FRAMES:], joints2d_lis[-MIN_NUM_FRAMES:],
                         bbox_lis[-MIN_NUM_FRAMES:], frame_lis[-MIN_NUM_FRAMES], orig_dim, 0)
...
vibe_results = get_vibe_results(pred_cam, pred_verts, pred_pose, pred_betas, pred_joints3d, joints2d_lis, bbox_lis,
                                frame_lis, orig_dim, 0)
